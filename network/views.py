...
def index(request):
    #cap number of posts per page at 10, retrieve all available posts
    posts_per_page = 10
    post_list = Post.objects.all()

    #all posts and posts per page as input for Django Paginator class, store in variable 'paginator'
    ...
    if request.method == "POST":
        ...
        # get the post text and store in a variable, if the post text is positive save the post under to user who posted
...
def user_view(request, user_name):
    # retrieve user name from registered Users
    ...
    # determine if follow button should be shown or not
    # follow button should only show if the logged in user is not the same as the profile being viewed
    ...
    # derive the nummber followers and following
...
def following(request):
    #get the posts by users which the current user is following
...
def post_edit(request, post_id):
    if request.method == "POST":
        # get post via that id
        post = Post.objects.filter(id=post_id)

        # check if the post exists, get that post's body and save
        if len(post) == 1:
            post_to_edit = post[0]
...
def like_post(request, post_id):
    if request.method == "POST":
        ...
        if len(post) == 1:
            ...
            # check if post is liked by the user, if its not add like when clicked, if it is remove like when clicked
